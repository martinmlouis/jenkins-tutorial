          ...
          container('kaniko'){
            ...
            withCredentials([usernamePassword(credentialsId: 'harbor-cred', usernameVariable: 'HARBOR_USERNAME', passwordVariable: 'HARBOR_PASSWORD')]) {
              sh """
                ...
                echo "{\\"auths\\":{\\"${HARBOR_URL}\\":{\\"username\\":\\"${HARBOR_USERNAME}\\",\\"password\\":\\"${HARBOR_PASSWORD}\\"},\\"docker.io\\":{\\"username\\":\\"martinlourduswamy\\",\\"password\\":\\"Kandikuppam@123\\"}}}" > /kaniko/.docker/config.json
